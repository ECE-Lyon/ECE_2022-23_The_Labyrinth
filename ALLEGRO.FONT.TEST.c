...
int current_Y = 0;
int menu_flag = 0;
int coord_MainTitleX = 1190;
int coord_MainTitleY = 203;
int coord_SubTitleX = 1174;
int coord_SubTitleY = 540;
...
int main(int argc, char **argv)
{
   ...
   ALLEGRO_FONT *custom_font = al_load_ttf_font("C:/Users/Anton/tes/Ashtray_W00_Full.ttf", 36, 0);
   ALLEGRO_COLOR text_color = al_map_rgb(253, 4, 54);
   ...
   /*if (!al_init()) 
   {
      fprintf(stderr, "Failed to initialize Allegro. Error code: %d\n", al_get_errno());
      return -1;
   }
   if (!al_init_image_addon())
   {
      fprintf(stderr, "Failed to initialize image addon. Error code: %d\n", al_get_errno());
      return -1;
   }
   if (!al_init_ttf_addon())
   {
      fprintf(stderr, "Failed to initialize font addon. Error code: %d\n", al_get_errno());
      return -1;
   }*/
   ...
   while (1)
   {
      ...
      if (current_index == 21)
      {
         ...
         al_draw_text(custom_font, text_color, coord_MainTitleX, coord_MainTitleY, ALLEGRO_ALIGN_LEFT, "It's player x turn !");
      }
      al_flip_display();
   }
